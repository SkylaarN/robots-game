...
public class ForwardCommand extends Command {
    ...
    public boolean execute(Robot target) {
        ...
        if (target.updatePosition(nrSteps) == Robot.Conditions.SUCCESS){
            ...
            data.put("message", target.getName() + " > " + positionText + " Moved forward by "+nrSteps+" steps.");
            ...
            data.put("message", target.getName() + " > " + positionText + " Sorry, I have encountered an obstacle.");
        }
        else {
            String positionText = "[" + target.getPosition().getX() + "," + target.getPosition().getY() + "]";
            //target.setStatus(target.getName() + " > " + positionText + " Sorry, I cannot go outside my safe zone.");
            //data.put("message", "Border");
            data.put("message", target.getName() + " > " + positionText + " Sorry, I cannot go outside my safe zone.");
        }
        reply.put("data", data);
